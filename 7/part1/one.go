...
func main() {
	...
	for _, round := range rounds {
		fiveMatches := isNumberOfKind(5, round.cards, []int{})
		fourMatches := isNumberOfKind(4, round.cards, fiveMatches)
		threeMatches := isNumberOfKind(3, round.cards, append(fiveMatches, fourMatches...))
		twoMatches := isNumberOfKind(2, round.cards, append(fiveMatches, append(fourMatches, threeMatches...)...))
		oneMatches := isNumberOfKind(1, round.cards, append(fiveMatches, append(fourMatches, append(threeMatches, twoMatches...)...)...))
...
func isNumberOfKind(number int, cards []int, alreadyHigher []int) []int {
	...
	for _, card := range cards {
		if !contains(alreadyHigher, card) {
			toProcess = append(toProcess, card)
		}
	}

	for i := 0; i < len(toProcess); i++ {
		occurrences[toProcess[i]]++
	}

	for card, occurrence := range occurrences {
