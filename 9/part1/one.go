...
func calculateSecret(index int, lineHistory LineHistory) {
	...
	for i := linesInHistory - 2; i >= 0; i-- {
		...
		lastInNextLine := nextLineInHistory.values[len(nextLineInHistory.values)-1]

		lastInThisLine := lineHistory.lines[i].values[len(lineHistory.lines[i].values)-1]

		toAddInThisLine := lastInNextLine + lastInThisLine
		lineHistory.lines[i].values = append(lineHistory.lines[i].values, toAddInThisLine)
